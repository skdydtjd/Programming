...
template <typename T>
auto create_bipartite_reference_graph2()
{
	Graph<T> G(7);

	map<unsigned, vector<pair<unsigned, T>>> edge_map;
	edge_map[1] = { {2,0} };
	edge_map[2] = { {1,0}, { 3,0 },{4,0},{5,0} };
	edge_map[3] = { {2,0},{4,0},{6,0} };
	edge_map[4] = { {2,0}, { 3,0 }, {5,0}, { 6,0 } };
	edge_map[5] = { {2,0},{4,0} };
	edge_map[6] = { {3,0}, { 4,0 } };
...
auto bipartite_check(const Graph<T>& G)
{
	...
	{
		colors::BLACK //다음 정점에 칠할 색상
	};

	vector<colors> vertex_colors(G.vetices(), colors::NONE);
	...
	while (!stack.empty())
	{
		...
		if (visited.find(current_vertex) == visited.end())
		{
			...
			//인접한 정점중에서 방문하지 않은 정점이 있다면 스택에 추가
		...
		//현재 정점이 이미 방문한 정점이고, 현재 칠할 색상과 같은 색상이 칠해져 있다면 이분 그래프가 아님
...
int main()
{
	...
	auto BG2 = create_bipartite_reference_graph2<T>();
	cout << "[입력 그래프]" << endl;
	cout << BG2 << endl;
